...
from tutorpicasso.picasso.share.domain.config_file import ConfigFile
from tutorpicasso.picasso.share.domain.config_file_validation_error import ConfigFileValidationError
from tutorpicasso.picasso.share.domain.config_setting import ConfigSetting
...
class ConfigThemesSetting(ConfigSetting):
    ...
    def validate(self) -> None:
        ...
        themes_settings_schema = Schema(
            {
                Optional("PICASSO_THEMES_NAME"): And(list, len),
                Optional("PICASSO_THEME_DIRS"): And(list, len),
                Optional("PICASSO_THEMES_ROOT"): And(str, len),
            },
            ignore_extra_keys=True
        )
        if "PICASSO_THEMES" in self.config:
            for theme in self.config["PICASSO_THEMES"]:
                try:
